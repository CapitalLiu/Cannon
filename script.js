...
class Stage
{
  constructor(scale, stageSize, onReady, showStats)
  {
    ...
    this.scene.background = new Color('#c74878');
    this.scene.fog = new Fog('#c74878', 50, 200);
    // this.scene.add(new AxisHelper());

    // LIGHTS

    this.scene.add(new HemisphereLight(0xfefeff, 0xeeeeff, 0.6                  ));
    ...
    for (let i = 0; i < 3; i++) {
      ...
      let light = new PointLight(color, 1, 50, 1.7);
...
function introCannon()
{
  ...
  cannonTL.to(steveEl, { y: '-=20', duration: 0.1, onComplete: () => {
      ...
      textEl.textContent = "但是等等，我们有这个 ！";
      textHighlightEl.textContent = "圣诞加农炮!!";
